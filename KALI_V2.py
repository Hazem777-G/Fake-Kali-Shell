...
# NEW: Initialize current directory and file system
current_dir = "Home"
file_system = {d: {} for d in chd}
file_system["Desktop"] = {"game.py": "print('Hello World')", "notes.txt": "Important meeting at 3 PM"}  # NEW
    ...
    try:
        if ter == "root":
            ...
            if cmd.startswith("apt install "):
                ...
                folder = cmd[3:].strip()
                if folder in chd:
                    global current_dir  # NEW
                    current_dir = folder  # NEW
                ...
                # NEW: Show files in current directory
                ...
                folder = cmd[3:].strip()
                if folder in chd:
                    # NEW: Show files in specified directory
                ...
                if delete in chd:
                    chd.remove(delete)
                    if delete in file_system:  # NEW
                        del file_system[delete]  # NEW
                    time.sleep(0.6)
                ...
                if filename:
                    file_system[current_dir][filename] = ""  # NEW
                ...
                if ' > ' in text:
                    ...
                    if filename:
                        file_system[current_dir][filename] = message  # NEW
